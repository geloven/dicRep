     ...
     for(x=0;x<rows;x++){
        if (x < g_max_display_rows) {
            ...
            if ((pageName == PAGE_NAME_ONE_VB) || (pageName == PAGE_NAME_ONE_VB_MISTAKE)) {
                ...
                if (isExistedInEntryList(list[x]["id_entry"], g_current_vb_id) == true){
                    checkBoxNode.checked = true;
                }else{
            ...
            if (type != TAB_IMPORT) {
                ...
                if (checkBoxNode != null){
                    if (type == TAB_VB) {
                        ...
                        if (isExistedInEntryList(list[startListIndex + x]["id_entry"], g_current_vb_id) == true){
                            checkBoxNode.checked = true;
                        }else{
                            checkBoxNode.checked = false;
                       }
...
function reviseVBWord(pageName)
{
    ...
    if (pageName == PAGE_NAME_ONE_VB_MISTAKE) {
        let removeFromMistakeBookNode = document.createElement("A");
        removeFromMistakeBookNode.setAttribute("id","id_revise_remove_mistake_record");
        removeFromMistakeBookNode.setAttribute("class", "page_down");
        removeFromMistakeBookNode.innerHTML = "<span class='glyphicon glyphicon-tag'></span> Remove from mistake collection";
        removeFromMistakeBookNode.setAttribute("href", "#id_vb_one");

        removeFromMistakeBookNode.onclick=function(){
            let indexNode = document.getElementById("id_index_string");
            let randomIndex = parseInt(indexNode.getAttribute("value"));
            let stringID = g_allwords_sort_list[randomIndex]["id_entry"];
            $.ajax({
             url: '/ajax/remove_vbMistakeRecord/',
             data: {
                'vb_id': g_current_vb_id,
                'entry_id' : stringID,
                'update_ip': g_my_ip_addr,
             ...
             dataType: 'json',
             success: function (data) {
                 let count = data.remove_count;
                 if (count > 0) {
                    $("#id_tip").html("<span style='color:green'> remove from mistake book.</span>");
                    //setVBMistakeInfo(g_current_vb_id, null, "");
                    refreshVBMistakeRecords_dataOnly();
                 }else{
                    $("#id_tip").html("<span style='color:red' > this vocabulary is already in mistake book.</span>");
                 }
             }
    ...
    switch(sortType){
        ...
        case ORDER_BY_INTERPRE:
            if (g_sort_interpre_flag == SORT_INTERPRE_FLAG_ASCEND) {
                  ...
                  g_allwords_sort_list.sort(function (a, b) {
                      if (a.interpre.toLowerCase() < b.interpre.toLowerCase()) {
                        return 1;
                      }
                      if (a.interpre.toLowerCase() > b.interpre.toLowerCase()) {
                        return -1;
        ...
        case ORDER_BY_UPDATEDATE:
            if (g_sort_updatedate_flag == SORT_UPDATEDATE_FLAG_ASCEND) {
                 ...
                 g_allwords_sort_list.sort(function (a, b) {
                      if (a.update_date.toLowerCase() < b.update_date.toLowerCase()) {
                        return 1;
                      }
                      if (a.update_date.toLowerCase() > b.update_date.toLowerCase()) {
                        return -1;
        ...
        case ORDER_BY_LANGCODE:
            if (g_sort_langcode_flag == SORT_LANGCODE_FLAG_ASCEND){
                 ...
                 g_allwords_sort_list.sort(function (a, b) {
                      if (a.lang_code.toLowerCase() < b.lang_code.toLowerCase()) {
                        return 1;
                      }
                      if (a.lang_code.toLowerCase() > b.lang_code.toLowerCase()) {
                        return -1;
